...
let showCollection = async () => {
        ...
        for (var i = 0; i < test.length; i++) {
            dbShow.innerHTML += '<p>Namn: ' + test[i].name + '| Dumhet: ' + test[i].dumhet + '| Är du cool?: ' + test[i].cool +  '</p>';
        }
};
...
let add = async () => {
    let response2 = await fetch('/mongodb/insert/[{"name":"' + name.value + '","dumhet":"' + dumhet.value + '","cool":"' + cool.value + '"}]');
